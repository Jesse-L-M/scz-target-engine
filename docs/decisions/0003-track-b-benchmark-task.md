...
3. fixture path: `data/benchmark/fixtures/scz_failure_memory_2025_02_01/`
...
2. slice-local `program_universe.csv` and `events.csv`
...
- snapshot, cohort, runner, and reporting keep one contract surface
...
- the benchmark registry now has multiple tasks under one suite and one frozen
  protocol question, so task resolution has to consider entity types and
  baseline sets
