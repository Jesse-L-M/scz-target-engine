...
def test_plan_public_benchmark_slices_discovers_honest_fixture_cutoffs() -> None:
    ...
    assert (
        "latest archived release 2024-06-20 is after requested cutoff 2024-06-15"
        in earliest_slice_exclusions["Open Targets"]
    )
    assert (
        "latest archived release 2024-07-15 is after requested cutoff 2024-06-15"
        in earliest_slice_exclusions["ChEMBL"]
    )


def test_materialize_public_benchmark_slices_writes_replayable_fixture_inputs(
    tmp_path: Path,
) -> None:
...
def test_public_slice_builder_reports_sparse_archive_coverage_limitation(
    ...
    archive_contents = "entity_id,entity_label,common_variant_support\nGENE_A,Gene A,0.9\n"
