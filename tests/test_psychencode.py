...
def test_fetch_psychencode_module_table_aggregates_duplicate_candidate_labels(
    tmp_path: Path,
) -> None:
    input_file = tmp_path / "candidate_gene_registry.csv"
    output_file = tmp_path / "module_evidence.csv"
    ...
    assert metadata["input_row_count"] == 6
    assert metadata["input_gene_count"] == 5
    ...
    assert drd2_entry["entity_id"] == "ENSGDRD2HIGH"
...
def test_build_module_member_gene_entries_keeps_support_on_one_duplicate_row() -> None:
    ...
    assert drd2_entry["entity_id"] == "ENSGDRD2RARE"
