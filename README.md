...
- Historical benchmark archives are fixture-scale. The checked-in archive set under `data/benchmark/fixtures/scz_small/` remains the small deterministic gene/module regression surface, while Track A public replay now consumes the dedicated checked-in historical archive surface under `data/benchmark/fixtures/scz_track_a_historical_replay/` and backfills ten honest slice-local intervention-object cuts under `data/benchmark/public_slices/`.
- Track A public slices now pin the checked-in denominator locally via
  ...
  executed and the result is **HOLD**. Available-now challenger baselines now run
  honestly at `intervention_object` grain through the frozen projection contract,
  and on the principal `2024-09-25` slice `schema_only` scores AP = 0.500 and
  `opentargets_only` scores AP = 0.333 against `v0_current` at AP = 0.125 and
  `v1_current` at AP = 0.167. The gate still fails because the cohort has only
  one positive intervention object, leaving the challenger 95% bootstrap
  intervals too wide to support a defensible material win. See
...
- `data/benchmark/generated/scz_failure_memory_2025_02_01/runner_outputs/metric_payloads/<run_id>/intervention_object/structural_replay/<metric>.json`: generated Track B `benchmark_metric_output_payload` files
- `data/benchmark/generated/scz_failure_memory_2025_02_01/runner_outputs/confidence_interval_payloads/<run_id>/intervention_object/structural_replay/<metric>.json`: generated Track B `benchmark_confidence_interval_payload` files
- `data/benchmark/generated/scz_failure_memory_2025_02_01/runner_outputs/track_b_case_outputs/<run_id>.json`: derived per-case Track B structural outputs
- `data/benchmark/generated/scz_failure_memory_2025_02_01/runner_outputs/track_b_confusion_summaries/<run_id>.json`: derived Track B confusion summaries
...
- `data/benchmark/generated/scz_failure_memory_2025_02_01/public_payloads/report_cards/scz_translational_suite/scz_failure_memory_track_b_task/scz_failure_memory_2025_02_01/<run_id>.json`: derived Track B public report card payload
- `data/benchmark/generated/scz_failure_memory_2025_02_01/public_payloads/leaderboards/scz_translational_suite/scz_failure_memory_track_b_task/scz_failure_memory_2025_02_01/intervention_object/structural_replay/<metric>.json`: derived Track B public leaderboard payload
- `data/benchmark/generated/scz_failure_memory_2025_02_01/public_payloads/error_analysis/scz_translational_suite/scz_failure_memory_track_b_task/scz_failure_memory_2025_02_01/<run_id>.md`: derived Track B markdown case review
...
- The canonical `scz_small` path remains a gene/module regression fixture. The checked-in public slices are the shipped Track A intervention-object replay path, they read their historical archive descriptors from `data/benchmark/fixtures/scz_track_a_historical_replay/`, and they execute the full available-now baseline set: `pgc_only`, `schema_only`, `opentargets_only`, `v0_current`, `v1_current`, `chembl_only`, and `random_with_coverage`.
...
- The Track A stop-go comparison was executed on 2026-04-08 and the result is **HOLD** (see `docs/decisions/0005-track-a-pr3-stop-go.md`). A follow-up feasibility audit determined the gate is **infeasible**: the honest archive universe contains exactly 1 independent positive event (xanomeline-trospium approval), so with 1 positive in 8 admissible entities the bootstrap CI always spans [0.0, ~1.0] and no honest reconfiguration can fix this. See `docs/decisions/0006-track-a-pr3-feasibility.md`.
