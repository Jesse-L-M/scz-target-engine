...
- Historical benchmark archives are fixture-scale. The checked-in archive set under `data/benchmark/fixtures/scz_small/` is still a small deterministic source surface, while Track A public replay now backfills ten honest slice-local intervention-object cuts under `data/benchmark/public_slices/`.
- Track A public slices now pin the checked-in denominator locally via
  ...
  honest positive intervention object each, but the real `v0_current` /
  `v1_current` stop-go comparison remains blocked because every evaluable slice keeps
  zero current-baseline coverage under the checked-in legacy archive universe. The
  later five cutoffs remain non-evaluable after the positive approval boundary lands.
...
- Those five evaluable slices are still not honest `v0_current` / `v1_current` stop-go comparisons. On the principal `2024-09-25` replay, `random_with_coverage` covers `8/8` intervention objects, while `v0_current` and `v1_current` both cover `0/8` because the checked-in legacy archives only expose `DISC1`, `CACNA1C`, and one unmatched module.
